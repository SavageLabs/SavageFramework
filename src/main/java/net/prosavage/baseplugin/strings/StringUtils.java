...
public class StringUtils {
    ...
    public static void setSlots(int slots) {
        ...
        try {
            Object invoke = Class.forName(Bukkit.getServer().getClass().getPackage().getName() + ".CraftServer").getDeclaredMethod("getHandle", (Class<?>[]) new Class[0]).invoke(Bukkit.getServer());
    ...
    public static void changeProperties(int slots) {
        Path resolve = Paths.get(BasePlugin.getInstance().getDataFolder().getParentFile().getAbsolutePath()).getParent().resolve("server.properties");
